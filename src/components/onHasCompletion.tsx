...
export default function onHasCompletion(cm, data, onHintInformationRender) {
  ...
  CodeMirror.on(data, 'select', (ctx, el) => {
    ...
    const description = ctx.description ? marked(ctx.description, { sanitize: true }) : '';
    const type = ctx.type && ctx.type !== 'undefined' ? '<span class="infoType">' + renderType(ctx.type) + '</span>' : '';

    information.innerHTML =
      '<div class="content">' + (description.slice(0, 3) === '<p>' ? '<p>' + type + description.slice(3) : type + description) + '</div>';
